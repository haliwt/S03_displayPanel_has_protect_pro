    ...
    [..]
      ...
      (#)When the processing function is called at first time after HAL_CRYP_Init()
       ...
       (#)To process a single message with consecutive calls to HAL_CRYP_Encrypt() or HAL_CRYP_Decrypt()
          without having to configure agModeln the Key or the Initialization Vector between each API call,
        ...
        if (hcryp->Init.KeyIVConfigSkip == CRYP_KEYIVCONFIG_ONCE)
        {
          ...
          else
          {
            /* If the Key and IV configuration has to be done only once
               ...
               to keep track it won't have to be done agModeln next time */
